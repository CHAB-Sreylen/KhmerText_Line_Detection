...
output_dir1 = "output/layout7/labels"
...
for i, paragraph in enumerate(paragraphs, start=1):
    ...
    confirm_bbox = draw_text_without_bbox(draw, (confirm_x, confirm_y), confirm_text, font=font_confirm_text, fill=header_color, label="text")
    ...
    text_confirm = 'សេចក្ដីប្រកាសព័ត៌មាន'
    ...
    # Read the title text from the text corpus
    title_corpus_path = "corpus/Title_text.txt"
    with open(title_corpus_path, 'r', encoding="utf-8") as title_file:
        title_text = title_file.read().strip()

    # Define the font for the title
    font_text_title = ImageFont.truetype(font_MPTCMoul, text_font_size)

    # Wrap the title text to a maximum width of 600 pixels
    max_title_width = 1900
    wrapped_title_lines = wrap_text(title_text, font_text_title, max_title_width, draw)

    # Calculate the total height of the wrapped lines to vertically center them
    title_line_spacing = 10
    total_title_height = sum(draw.textbbox((0, 0), line, font=font_text_title)[3] for line in wrapped_title_lines) + \
                        (len(wrapped_title_lines) - 1) * title_line_spacing

    # Set the starting Y position for the title rendering
    title_start_y = subtitle_y + 100
    current_y = title_start_y

    # Render each line of the wrapped title text
    for line in wrapped_title_lines:
        line_width = draw.textlength(line, font=font_text_title)
        title_x = (a4_width_px - line_width) / 2  # Center the text horizontally
        draw_text_without_bbox(draw, (title_x, current_y), line, font=font_text_title, fill=text_color, label="text")
        current_y += (draw.textbbox((0, 0), line, font=font_text_title)[3] - draw.textbbox((0, 0), line, font=font_text_title)[1]) + title_line_spacing
    ...
    draw_text_without_bbox(draw, (Date_x, Date_y), footer_text, font=font_footer_text, fill=text_color, label="footer")
    ...
    output_path = os.path.join(output_dir, f"kh_data_{i}.jpg")
