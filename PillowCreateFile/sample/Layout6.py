...
stamp = Image.open('img/stamp.png').convert('RGBA').resize((650, 500), Image.LANCZOS)
stamp_x, stamp_y = int(a4_width_px /2) +200 , + Address_y+200
...
# First text
text_address_1 = '- វិទ្យាសាស្រ្តកុំព្យូទ័រ (Computer Science)'
Address_x, Address_y = 350, Address_y + 100
font_text_address = ImageFont.truetype(font_MPTC, text_font_size)

# Draw the first text
draw_text_with_bbox(draw, (Address_x, Address_y), text_address_1, font=font_text_address, fill=text_color)

# Calculate the width of the first text using textbbox
bbox = draw.textbbox((Address_x, Address_y), text_address_1, font=font_text_address)
text_width_1 = bbox[2] - bbox[0]  # width = right - left

# Second text (placed right after the first one)
text_address_2 = 'www.dsdp.gov.kh/registration'
Address_x2 = Address_x + text_width_1 + 10  # Add a 10-pixel space between texts
draw_text_with_bbox(draw, (Address_x2, Address_y), text_address_2, font=font_text_address, fill=header_color)

text_address = 'អាស្រ័យហេតុនេះ សូមសិស្ស និង​សាធារណជនទាំំងអស់មេត្តាជ្រាបជាព័តមាន។'
Address_x, Address_y = 270 , Address_y+ 100
font_text_address = ImageFont.truetype(font_MPTC, text_font_size)
draw_text_with_bbox(draw, (Address_x, Address_y), text_address, font=font_text_address, fill=text_color)


qr_info = Image.open('img/Gov_qr.png').convert('RGBA').resize((300, 300), Image.LANCZOS)
info_qr_x, info_qr_y = 270, Address_y + 200
...
text_address = 'ស្គេនដើម្បីចុះឈ្មោះ'
qr_font_size = 37
Address_x, Address_y = 270 ,info_qr_y+ 320
font_text_address = ImageFont.truetype(font_MPTC, qr_font_size)
draw_text_with_bbox(draw, (Address_x, Address_y), text_address, font=font_text_address, fill=text_color)


output_path = "output/Layout_with_bounding_boxes6.jpg"
